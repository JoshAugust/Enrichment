...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const [totalResult, byStatusResult, byStateResult, verifiedResult, avgScoreResult, addedTodayResult, addedThisWeekResult] =
      await Promise.all([
        db.select({ count: sql<number>`count(*)` }).from(leads),
        db.select({ status: leads.status, count: sql<number>`count(*)` }).from(leads).groupBy(leads.status),
        db.select({ state: leads.state, count: sql<number>`count(*)` })
          .from(leads)
          .where(isNotNull(leads.state))
          .groupBy(leads.state)
          .orderBy(sql`count(*) DESC`)
          .limit(20),
        db.select({ verified: leads.verified, count: sql<number>`count(*)` }).from(leads).groupBy(leads.verified),
        db.select({ avg: sql<number>`avg(${leads.quality_score})` }).from(leads).where(isNotNull(leads.quality_score)),
        db.select({ count: sql<number>`count(*)` }).from(leads).where(gte(leads.created_at, startOfToday)),
        db.select({ count: sql<number>`count(*)` }).from(leads).where(gte(leads.created_at, startOfWeek)),
      ]);
