/**
 * talking-points-generator.js — Generates company-specific talking points
 *
 * Exportable module version of scripts/generate-talking-points.js
...
function generateForOperator(company) {
  ...
  if (company.investors && company.investors.length > 5) {
    points.push(`I noticed you've worked with ${company.investors.slice(0, 80)} — we're already in conversations with similar firms on how this fits into GPU-backed lending.`);
  }
...
function generateTalkingPoints(company) {
  ...
  let points;
  if (type === 'lender') points = generateForLender(company);
  else if (type === 'arranger') points = generateForArranger(company);
  else points = generateForOperator(company);
...
/**
 * Generate and persist talking points for all companies (or a subset).
 */
function generateAll(opts = {}) {
  const { priority, limit } = opts;
  ...
  try { db.exec('ALTER TABLE companies ADD COLUMN talking_points TEXT'); } catch (_) {}

  const stmt = db.prepare("UPDATE companies SET talking_points = ?, updated_at = datetime('now') WHERE id = ?");
  let updated = 0;

  for (const company of companies) {
    try {
      const { points, followups } = generateTalkingPoints(company);
      stmt.run(JSON.stringify({ points, followups, generated_at: new Date().toISOString() }), company.id);
      updated++;
    } catch (_) {}
  }

  return { total: companies.length, updated };
}

module.exports = { generateTalkingPoints, generateAll };
